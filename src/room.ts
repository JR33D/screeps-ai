...
import { ControllerUpgrader, SuperControllerUpgrader } from "./builder";
...
import { CreepTypes } from "./enums/creep-type";
...
export class MyRoom {
     ...
     * @returns { Memory}
     */
    private constructEmptyRoom(): Memory {
        return {
            active: [],
            building: [],
            queue: [],
...
// TODO: use memory to get the structures
    ...
    private findRoomCreeps():Memory[] {
        return _.filter<Memory>(Memory.creeps, (creepMemory:Memory) => {
     ...
     */
    private canCreateCreep(spawn:Spawn, creepMemory: Memory):number {
    ...
    private enqueueCreep(type:CreepTypes) {
        ...
        if (this.roomConfig['creeps']['priorityList'] && this.roomConfig['creeps']['priorityList'].indexOf(type) !== -1) {
            this.roomMemory.priorityQueue.push(new Memory());
        } else {
            this.roomMemory.queue.push(new Memory());
        }
    }
